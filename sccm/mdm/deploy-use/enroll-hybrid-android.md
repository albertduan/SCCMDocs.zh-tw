...
本主題可協助 IT 系統管理員啟用 Android 和 Android for Work 裝置的混合式註冊。 接著，便可使用 Configuration Manager 藉由已設定「Microsoft Intune 訂閱」來管理裝置。 使用者可以從 Google Play 下載 Android 公司入口網站應用程式，以註冊 Android (包含 Samsung KNOX Standard) 和 Android for Work 裝置。 如果您 Configuration Manager 系統管理員，您便可以管理合規性設定、抹除或刪除 Android 裝置、部署應用程式，以及收集軟體和硬體清查。 如果 Android 裝置上未安裝 Android 公司入口網站應用程式，您將不會擁有所有管理功能 (例如清查和相容性設定)，但是仍然可以將應用程式部署到 Android 裝置。  

## <a name="enable-android-enrollment"></a>啟用 Android 註冊  
下列步驟可讓 Configuration Manager 無須工作設定檔 (亦即「傳統 Android」註冊) 即可管理 Android 裝置。

1.  **必要條件** - 請先完成[設定混合式 MDM](setup-hybrid-mdm.md) 中的必要條件和程序，才能為任何平台設定註冊。  
2.  在 Configuration Manager 主控台的 [系統管理] 工作區中，選取 [概觀] > [雲端服務] > [Microsoft Intune 訂閱]，然後選取您的 Intune 訂閱。  
3.  在 [常用] 索引標籤上的 [訂閱] 群組中，選取 [設定平台] > [Android]。  
...
下列步驟可讓 Configuration Manager 無須工作設定檔 (亦即「傳統 Android」註冊) 即可管理 Android 裝置。
 ...
 1. 在 https://accounts.google.com/SignUp 建立 Google 帳戶以作為 Android for Work 系統管理員帳戶，或是使用將與這個 Intune 租用戶的所有 Android for Work 管理工作建立關聯的帳戶進行登入。 這可能是管理 Android 裝置的系統管理員所共用的 Google 帳戶。 這是您的組織在 Play for Work 主控台中管理及發行應用程式所使用的 Google 帳戶。 您將使用此帳戶來核准 Play for Work 市集中的應用程式，因此請記下帳戶名稱和密碼。
 2. 將 Google 帳戶繫結至 Configuration Manager 中所管理的 Intune 租用戶，以啟用 Android 註冊：
   1. 在 Configuration Manager 主控台的 [系統管理] 工作區中，選取 [概觀] > [雲端服務] > [Microsoft Intune 訂閱]，然後選取您的 Intune 訂閱。
   2. 在 [常用] 索引標籤上的 [訂閱] 群組中，選取 [設定平台] > [Android for Work]。
   3. 在對話方塊中，選取 [在 Intune 主控台中設定 Android for Work]。 Intune 主控台會在網頁瀏覽器中開啟。
   4. 使用 Intune 系統管理員認證來登入 Intune 入口網站。
   5. 選取 [設定] 以開啟 Google Play 的 Android for Work 網站。
 ...
 3. 當您返回 Intune 入口網站時，Android for Work 已啟用，並且會有三個 Android for Work 裝置註冊選項︰
   ...
   - **將這些群組中僅限使用者的受支援裝置作為 Android for Work 管理** - (僅針對某些群組啟用) 可讓您將 Android for Work 管理目標指向一組有限的使用者。 只有註冊支援 Android for Work 之裝置的所選群組成員，才能註冊為 Android for Work 裝置。 所有其他成員則會註冊為 Android 裝置。

> [!NOTE]
> 一項已知的問題會造成 [將這些群組中僅限使用者的受支援裝置作為 Android for Work 管理]**Android** 選項無法正常運作。 指定的 Azure AD 群組中的使用者裝置將註冊為 Android，而不是 Android for Work。 若要啟用 Android for Work，您必須使用 [將所有受支援的裝置作為 Android for Work 管理]。
...
### <a name="enroll-an-android-for-work-device"></a>註冊 Android for Work 裝置
 使用者註冊 Android for Work 裝置的方式與註冊 Android 裝置類似。 使用者可以在其行動裝置上下載並安裝 Android 的「公司入口網站」應用程式。 該應用程式會提示他們在註冊的過程中建立工作設定檔。  建立工作設定檔之後，使用者必須切換到受管理的「公司入口網站」版本。 受管理公司入口網站會在右下角標上小型橘色公事包。
