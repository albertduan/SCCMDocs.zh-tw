---
title: "建立全域條件 | System Center Configuration Manager"
...
1.  在 Configuration Manager 主控台中，按一下 [軟體程式庫] > [應用程式管理] > [全域條件]。  
...
## <a name="configure-rules-for-the-global-condition"></a>設定全域條件的規則  
 定義全域條件規則的程序會依您是設定設定值或運算式而有所不同。 使用此處適用的程序來設定全域條件的設定值或運算式。  

### <a name="to-configure-a-setting-for-the-global-condition"></a>設定全域條件的設定值  
...
### <a name="configure-an-expression-for-the-global-condition"></a>設定全域條件的運算式  
