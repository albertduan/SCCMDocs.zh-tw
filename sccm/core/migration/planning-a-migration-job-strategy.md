...
---
# <a name="planning-a-migration-job-strategy-in-system-center-configuration-manager"></a>規劃 System Center Configuration Manager 中的移轉作業策略

*適用於：System Center Configuration Manager (最新分支)*

使用移轉作業設定您要移轉至 System Center Configuration Manager 環境中的特定資料。 移轉作業可識別您要移轉的物件，而且會在目的地階層中的頂層站台執行。 您可以在每個來源站台上設定一項或多項移轉作業。 這樣可讓您一次移轉所有物件，或是在每項作業中移轉有限的資料子集。  
 ...
 您在各階層之間移轉用戶端之前，請先確定用戶端使用的物件已移轉，且這些物件都在目的地階層中。 例如，當您從 Configuration Manager 2007 SP2 來源階層移轉時，可能有部署至包含用戶端之自訂集合的內容公告。 在此案例中，您應在移轉用戶端之前移轉集合、公告及關聯內容。 這是因為，如果內容、集合和公告未在用戶端移轉之前移轉，此資料就無法與目的地階層中的用戶端產生關聯。 如果用戶端未與之前執行的公告和內容相關的資料產生關聯，用戶端就可能收到要在目的地階層中安裝的內容，但該內容並非必要。 若用戶端在資料移轉之後移轉，用戶端就會與此資料和公告產生關聯，而除非公告為週期性，否則就不會再收到已移轉公告的此內容。  
...
-   [規劃集合移轉作業](#About_Collection_Migration)  
...
##  <a name="a-nametypesofmigrationa-types-of-migration-jobs"></a><a name="Types_of_Migration"></a> 移轉作業類型  
 Configuration Manager 支援下列移轉作業類型。 每一種作業類型的設計都是為了幫助您定義可包含在該作業內的物件：  
...
##  <a name="a-nameaboutmigrationjobsa-general-planning-for-all-migration-jobs"></a><a name="About_Migration_Jobs"></a> 所有移轉作業的一般規劃  
 使用 [建立移轉作業精靈] 建立移轉作業將物件移轉至您的目的地階層。 您建立的移轉作業類型會決定可移轉的物件。 您可以建立並使用多項移轉作業從相同的來源站台，或是從多個來源站台移轉資料。 使用某一種類型的移轉作業並不會阻止您使用另一種類型的移轉作業。  

 成功執行移轉作業後，其狀態會列出為 [已完成]  且無法再次執行。 不過，您可以建立新的移轉作業來移轉原始作業所移轉的任何物件，而且新的移轉作業也可包含其他物件。 當您建立其他移轉作業時，之前已移轉物件的狀態會顯示為 [已移轉] 。 您可以選取這些物件並再次移轉；不過，除非物件已在來源階層中更新，否則不需要再次移轉這些物件。 如果物件最初移轉後已在來源階層中更新，當您使用移轉作業類型 [移轉後修改的物件] 時可以識別該物件。  
...
> [!NOTE]  
>  在您使用 [清理移轉資料]  動作完成移轉後，可以重新設定相同的階層做為目前的來源階層，以便再次顯示您之前移轉的物件。  

 在 Configuration Manager 主控台中選取移轉作業，然後按一下 [作業中的物件] 索引標籤，即可檢視任何移轉作業中包含的物件。  
...
### <a name="site-ownership-for-migrated-content"></a>已移轉內容的站台擁有權  
 ...
 雖然藉由使用資料庫複寫就能在目的地階層中的所有站台上共用內容相關資訊，您指派至主要站台後再部署到其他主要站台發佈點的內容，仍會使用以檔案為基礎的複寫來傳輸。 這項傳輸是透過管理中心網站路由傳送至其他各主要站台。 在您將站台指派為內容擁有者時，藉由移轉前或移轉期間，將您規劃發佈至多個主要站台的套件集中化，就能降低資料傳輸量，使用較低的網路頻寬。  

### <a name="configure-role-based-administration-security-scopes-for-migrated-data"></a>為移轉的資料設定以角色為基礎的系統管理安全性範圍  
 ...
 在您設定移轉作業之前，請先檢閱以角色為基礎的系統管理如何在 System Center Configuration Manager 中運作，並於必要時，為您移轉的資料設定一或多個安全性範圍，以便控制能夠存取目的地階層中已移轉物件的人員。  
...
### <a name="scheduling-migration-jobs"></a>排程移轉作業  
 根據預設，移轉作業會在建立之後立即執行。 不過，您可以在建立作業時或之後藉由編輯作業的內容，指定何時執行移轉作業。 您可以排程依照下列時間執行移轉作業。  
...
##  <a name="a-nameaboutcollectionmigration-a-planning-for-collection-migration-jobs"></a><a name="About_Collection_Migration "></a> 規劃集合移轉作業  
 ...
 使用以下各節的資訊來瞭解其他設定，以套用至依集合進行的移轉作業。  

### <a name="excluding-objects-from-collection-migration-jobs"></a>從集合移轉作業排除物件  
...
-   包含不同資源類型集合參照的集合。 例如某個以裝置為基礎的集合，集合中包含以使用者為主的集合之子集合或連結。 本範例中僅頂層集合會進行移轉。  
...
### <a name="collection-dependencies-and-include-objects"></a>集合相依性與包含物件  
 當您在建立移轉作業精靈中指定要移轉的集合時，系統將自動選取任何相依的集合以包含到移轉作業中。 此行為可確保在移轉之後所有必要的資源仍可使用。  
...
### <a name="collection-limiting"></a>集合限制  
 ...
 例如在 Configuration Manager 2007 中，將於建立集合的所在站台與子站台上為集合進行評估。 公告可能僅會部署至子站台，如此便可將該公告的範圍限制於該子站台。 相較之下，System Center Configuration Manager 集合則會在每個站台進行評估，之後才會為每個站台評估相關聯的公告。 集合限制可供您依據另一個集合來縮小集合成員，以避免加入非預期的集合成員。  

### <a name="site-code-replacement"></a>站台碼取代  
 若您要移轉的集合包含可識別 Configuration Manager 2007 站台的準則，那麼您必須在目的地階層中指定特定的站台。 如此可確保移轉後的集合在目的地階層中仍可運作，且其範圍不會變大。  

### <a name="specify-behavior-for-migrated-advertisements"></a>指定移轉後公告的行為  
 根據預設，依集合進行的移轉作業會停用移轉至目的地階層的公告。 其包含與公告相關聯的任何程式。 若您所建立之集合型移轉作業中包含公告，您可在 [建立移轉作業精靈] 的 [設定]  頁面上，看到 [在公告移轉後啟用 Configuration Manager 部署程式]  的選項。 若您選取此選項，將在移轉後啟用與公告相關聯的程式。 建議的最佳作法是不要選取此選項，而是在完成移轉後驗證要接收這些程式的用戶端，然後才啟用這些程式。  

> [!NOTE]  
>  只有在您所建立集合型移轉作業中包含公告時，才會看到 [在公告移轉後啟用 Configuration Manager 部署程式]  的選項。  

 若要在移轉後啟用程式，請於程式內容的 [進階]  標籤中取消選取 [在程式公告所在電腦上停用該程式]  選項。  

##  <a name="a-nameaboutobjectmigrationa-planning-for-object-migration-jobs"></a><a name="About_Object_Migration"></a> 規劃物件移轉作業  
...
##  <a name="a-nameaboutobjectmigrationsa-planning-for-previously-migrated-object-migration-jobs"></a><a name="About_Object_Migrations"></a> 規劃先前移轉的物件移轉作業  
 ...
 當您選取此作業類型時，[建立移轉作業精靈] 之 [設定]  頁面上的衝突解決行為，會設定為覆寫先前移轉的物件，而且此設定無法變更。  
