...
- ***[新增]*** 管理主控台：
    ...
    -  主控台連線的相關統計資料 (作業系統版本、語言、SKU 和架構；系統記憶體、邏輯處理器計數、Connect 網站識別碼、所安裝的 .NET 版本和主控台語言套件)
...
-   **應用程式管理：**
  ...
  -   ***[已更新]*** 組織內所使用之部署類型的基本使用方式/目標資訊 (目標使用者與裝置、必要與可用、通用應用程式)  

  -  ***[已更新]*** 應用程式部署資訊 (安裝/解除安裝、需要核准、啟用/停用使用者互動、相依性、取代)  
...
-   **相容性設定：**
    ...
    -   ***[已更新]*** 所部署之簡單憑證註冊通訊協定、VPN、WiFi、憑證 (.pfx) 和相容性原則範本的計數   

    -  ***[新增]*** SCEP 憑證、VPN、Wifi、憑證 (.pfx) 和相容性原則部署的計數 (依平台)
...
-   **SQL/效能資料：**
    ...
    -   ***[已更新]*** 集合評估統計資料 (查詢時間、已指派與未指派計數、依類型的計數、ID 變換和規則使用方式)
...
-   ***[新增]*** 軟體更新點同步的軟體更新產品
-   ***[新增]*** 相容性設定︰SCEP、VPN、Wifi 和相容性原則範本設定詳細資料
