...
> [!IMPORTANT]  
> System Center Configuration Manager 支援 SHA-2 憑證，並使用 SHA-2 憑證帶來重要的安全性優勢。 因此，我們建議︰
...
|雲端發佈點|伺服器驗證|**Web 伺服器**|**[增強金鑰使用方法]** 值必須包含 **[伺服器驗證 (1。3。6。1。5。5。7。3。1)]**。<br /><br /> [主體名稱] 必須包含 FQDN 格式的客戶定義服務名稱及網域，做為特定雲端架構發佈點執行個體的 [一般名稱]。<br /><br /> 私密金鑰必須可匯出。<br /><br /> 支援 SHA-2 雜湊演算法。<br /><br /> 支援的金鑰長度：2048 位元。|此服務憑證用來向 Configuration Manager 用戶端驗證雲端架構發佈點服務，以及將使用安全通訊端層 (SSL) 在它們之間傳送的所有資料加密。此憑證必須以公開金鑰憑證標準 (PKCS #12) 格式匯出，並且必須知道密碼，以便在您建立雲端架構發佈點時，可以將它匯入。<br /><br /> **注意：** 此憑證會搭配 Windows Azure 管理憑證使用。 |  
