...
## <a name="fallback"></a>後援  
 從 1610 版開始，關於用戶端如何尋找有內容的發佈點的幾個概念已經改變，包括後援。 請使用下列適用您所用版本的資訊︰

**1610 版和更新版本︰**   
若用戶端無法從與其目前界限群組相關聯的發佈點找不到內容，則可後援使用與芳鄰界限群組相關聯的內容來源位置。 要用於後援的芳鄰界限群組，必須有與用戶端的目前界限群組有已定義的關聯性。 此關聯性包括一段設定的時間：用戶端在此時間內於本機找不到內容，才能將來自芳鄰界限群組的內容來源納入其搜尋。
...
從 1610 版開始，關於用戶端如何尋找有內容的發佈點的幾個概念已經改變，包括內容來源的網路連線速度。 請使用下列適用您所用版本的資訊︰
