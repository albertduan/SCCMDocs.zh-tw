...
-   熟悉 SQL Server 的文件，其中涵蓋可用性群組的使用方式，以及完成下列案例所需的相關程序。
...
若要完成此程序，您使用的帳戶必須是：
-   將會在可用性群組中之每部電腦上的「本機 Administrator」群組成員。
-   每個將裝載站台資料庫之 SQL Server 執行個體上的 **sysadmin**。
...
5.    檢查每個複本上的設定：   
  ...
  3.    如果必須設定次要複本的設定，由於您只能設定主要複本的資料庫，因此必須手動將主要複本容錯移轉至次要複本，然後才能繼續進行。 如需詳細資訊，請參閱 SQL Server 文件中的[執行可用性群組規劃的手動容錯移轉](/sql/database-engine/availability-groups/windows/perform-a-planned-manual-failover-of-an-availability-group-sql-server)。
...
## <a name="add-and-remove-replica-members"></a>新增和移除複本成員  
當站台資料庫裝載於可用性群組中時，可使用下列程序來新增或移除複本成員。 Configuration Manager 支援單一主要節點，以及最多兩個次要節點。
