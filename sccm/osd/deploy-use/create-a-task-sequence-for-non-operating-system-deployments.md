...
工作順序的強大功能在於其彈性，以及您可以如何使用這些工作順序來設定用戶端設定、發佈軟體、更新驅動程式、編輯使用者狀態，並執行與作業系統部署無關的其他工作。 您可以建立自訂工作順序以新增任何數量的工作。 您可以建立自訂工作順序以新增任何數量的工作。 Configuration Manager 支援使用非作業系統部署的自訂工作順序。 不過，如果工作順序導致不想要或不一致的結果，請查看如何簡化作業。 做法是使用較簡單的步驟、將動作分成多個工作順序，或採取分段方式來建立和測試工作順序。
